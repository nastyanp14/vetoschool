...
DECLARE
  ...
  should_repair_missing_credit BOOLEAN := false;
BEGIN
  ...
  IF inserted_payment_id IS NULL THEN
    SELECT
      public.profiles.stripe_customer_id IS NULL
      AND public.profiles.stripe_subscription_id IS NULL
      AND COALESCE(public.profiles.lessons_total, 0) = 0
      AND COALESCE(public.profiles.lessons_remaining, 0) = 0
    INTO should_repair_missing_credit
    FROM public.profiles
    WHERE public.profiles.id = p_user_id;

    UPDATE public.profiles
    SET
      stripe_customer_id = COALESCE(NULLIF(public.profiles.stripe_customer_id, ''), p_stripe_customer_id),
      stripe_subscription_id = COALESCE(NULLIF(public.profiles.stripe_subscription_id, ''), p_stripe_subscription_id),
      stripe_price_id = COALESCE(NULLIF(public.profiles.stripe_price_id, ''), p_stripe_price_id),
      subscription_status = p_subscription_status,
      plan_id = COALESCE(NULLIF(public.profiles.plan_id, ''), p_plan_id),
      lesson_format = COALESCE(NULLIF(public.profiles.lesson_format, ''), p_lesson_format),
      lessons_total = CASE
        WHEN should_repair_missing_credit THEN p_lessons_total
        ELSE public.profiles.lessons_total
      END,
      lessons_remaining = CASE
        WHEN should_repair_missing_credit THEN p_lessons_total
        ELSE public.profiles.lessons_remaining
      END,
