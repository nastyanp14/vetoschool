    ...
    <DialogPrimitive.Root open={preferencesOpen} onOpenChange={open => { if (!open) closePreferences(); }}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className="fixed left-1/2 top-1/2 z-[71] max-h-[calc(100vh-2rem)] w-[calc(100vw-1.5rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-3xl border border-pink-100 bg-white p-5 shadow-2xl shadow-purple-200/50 outline-none dark:border-purple-700 dark:bg-[#1b0c2f] dark:shadow-black/40 sm:p-6"
        >
          <motion.div
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <DialogPrimitive.Title className="font-display text-2xl font-black text-purple-700 dark:text-purple-100">
                  {t(lang, 'cookie_modal_title')}
                </DialogPrimitive.Title>
                <DialogPrimitive.Description className="mt-2 font-body text-sm font-semibold leading-relaxed text-purple-500 dark:text-purple-200">
            ...
            <div className="space-y-3">
              {categories.map(category => (
                <section
                  ...
                  className="rounded-3xl border border-purple-100 bg-gradient-to-br from-white to-purple-50/60 p-4 dark:border-purple-700 dark:from-[#241331] dark:to-[#1c1029]"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <h3 id={`cookie-category-${category.id}`} className="font-display text-base font-black text-purple-700 dark:text-purple-100">
                        {category.title}
                      </h3>
                      <p id={`cookie-category-${category.id}-desc`} className="mt-1 font-body text-sm font-semibold leading-relaxed text-purple-500 dark:text-purple-200">
                        {category.desc}
                      </p>
                      <p className="mt-2 inline-flex items-center gap-1 rounded-full bg-pink-50 px-3 py-1 text-xs font-black text-purple-500 dark:bg-white/10 dark:text-purple-200">
            ...
            <div className="mt-5 grid gap-2 sm:grid-cols-2">
              ...
              <button
                ...
                className="rounded-2xl border border-pink-200 bg-pink-50 px-4 py-3 text-sm font-black text-purple-700 transition hover:-translate-y-0.5 hover:bg-pink-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-pink-200 dark:border-purple-700 dark:bg-white/10 dark:text-purple-100"
              >
                {t(lang, 'cookie_reject_optional')}
              </button>
              <DialogPrimitive.Close
                className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-black text-purple-700 transition hover:-translate-y-0.5 hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200 dark:border-purple-700 dark:bg-white/10 dark:text-purple-100"
              >
                {t(lang, 'cookie_close')}
              </DialogPrimitive.Close>
            </div>
