import { planIdFromStripePriceId, stripePlanConfig, stripePriceIdsByPlan } from './stripePrices';
...
function stripePortalReturnUrl(request: Request) {
  return new URL(DEFAULT_STRIPE_PORTAL_RETURN_PATH, request.url).toString();
}
...
export async function handleCreateStripeCheckoutSession(request: Request, env: StripeCheckoutEnv) {
  ...
  if (!secretKey) {
    return jsonResponse({ error: 'Stripe secret key is not configured on the server.' }, 500);
  }
  ...
  if (!isPricingPlanId(body.planId)) {
    ...
    return jsonResponse({ error: 'Unknown Vetoschool plan.' }, 400);
  }
...
export async function handleCreateStripePortalSession(request: Request, env: StripeCheckoutEnv) {
  ...
  const portalBody = new URLSearchParams({
    ...
    return_url: stripePortalReturnUrl(request),
  });
